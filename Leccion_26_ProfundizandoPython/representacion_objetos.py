# Representación de objetos (str, repr, format)
# print(dir(object))

class Persona:
    def __init__(self, nombre, apellido):
        self.nombre = nombre
        self.apellido = apellido

    # repr, está enfocado a los programadores (ofrece información)
    def __repr__(self):
        return f'{self.__class__.__name__}(nombre:{self.nombre}, apellido:{self.apellido})'

    # str, enfocado para usuario final u otros sistemas
    # implementación por default llama al metodo str
    def __str__(self):
        return f'{self.__class__.__name__}: {self.nombre} {self.apellido}'

    # format su implementación por default es str
    # se manda a llamar al usar f-string
    def __format__(self, format_spec):
        return f'{self.__class__.__name__} con nombre {self.nombre} y apellido {self.apellido}'


persona1 = Persona('Juan','Perez')
# repr (!r)
print(f'Mi objeto persona1: {persona1!r}') # Se llama el metodo repr en lugar de str
# str (forma automática)
print(persona1.__str__())
# format (str por default)
print(f'{persona1}')