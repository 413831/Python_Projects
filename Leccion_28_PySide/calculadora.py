...
class Calculadora(QMainWindow):
    ...
    def _crear_botones(self):
        ...
        # La posición es una tupla con dos valores (renglon-columna)
        for texto_boton, posicion in botones.items():
            ...
            if texto_boton not in {'=', 'C'}:
                self.botones[texto_boton].clicked.connect(partial(self._construir_expresion,texto_boton))
            # Publicamos el botón en el grid layout
            layout_botones.addWidget(self.botones[texto_boton], posicion[0], posicion[1])
        # Conectamos el botón de limpiar texto (C=Clear)
    ...
    #     # Recorrer cada boton del diccionario (kay:value) (texto:PushButton)
    ...
    def _construir_expresion(self, texto_boton):
        ...
        # Actualizamos la línea de entrada
