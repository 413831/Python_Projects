...
class Rectangulo:
    ...
    def __repr__(self):
        return f'Rectángulo({self.sup_izq!r}, {self.inf_der!r})'


rect_a = Rectangulo(Punto(0,1), Punto(3,4))
#  Copia superficial (Shallow
rect_b = copy.copy(rect_a)
print(f'Copia Superficial rectángulos')
print(f'Rectangulo a: {rect_a}')
print(f'Rectangulo b: {rect_b}')
# debido a que la copia fue superficial, un cambio en un punto afecta al otro
rect_a.inf_der.y = 6
print(f'Cambio en un punto afecta al otro rectángulo (shallow copy)')
print(f'Rectangulo a: {rect_a}')
print(f'Rectangulo b: {rect_b}')

# Creación copia profunda (deep copy)
rect_c = copy.deepcopy(rect_a)
# Modificamos un valor en un punto, no afecta al otro rectángulo (copia profunda)
rect_c.sup_izq.x = 2
print(f'Rectangulo a: {rect_a}')
